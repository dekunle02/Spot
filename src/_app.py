...
from utils import parser, composer
...
(
    FIRST_NAME_EDIT,
    FIRST_NAME_EDIT_PROMPT,
    LAST_NAME_EDIT,
    LAST_NAME_EDIT_PROMPT,
    HOSPITAL_NAME_EDIT,
    HOSPITAL_NAME_EDIT_PROMPT,
    SIGNATURE_EDIT,
    SIGNATURE_EDIT_PROMPT,
) = range(4, 12)
...
def general(update: Update, context: CallbackContext) -> int:
    ...
    if user != None:
        update.message.reply_text(f"Hello {user.first_name} " + Replies.IDLE_MESSAGE)
    else:
        update.message.reply_text(Replies.WELCOME_MESSAGE)
...
def info(update: Update, context: CallbackContext):
    ...
    if user:
        ...
        update.message.reply_text(Replies.UNKNOWN_USER_INFO_ASK)


def sam(update: Update, context: CallbackContext) -> int:
    update.message.reply_text(Replies.SAMAD_ASK)
    return SAM_MESSAGE
...
def first_name(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(text=Replies.LAST_NAME_ASK)
    return LAST_NAME
...
def signature(update: Update, context: CallbackContext) -> int:
    ...
    fire.add_user(user)
    update.message.reply_text(Replies.SETUP_COMPLETE_MESSAGE)
    return ConversationHandler.END
...
def edit(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            ...
            input_field_placeholder=f"Type {Options.YES} or {Options.NO}",
        ),
    )
...
def night_prompt(update: Update, context: CallbackContext) -> int:
    ...
    if prompt_answer == Options.NO:
        ...
        context.bot.send_message(
            chat_id=MY_TELEGRAM_ID,
            text=f"{sender.id} {sender.first_name} made a timesheet!",
        )
        return ConversationHandler.END

    update.message.reply_text(
        Replies.DISTURBANCE_DAY_ASK,
...
def night_reason(update: Update, context: CallbackContext) -> int:
    ...
    reply_keyboard = [[Options.NO, Options.YES]]
    update.message.reply_text(
        text=Replies.DISTURBANCE_ASK_ANOTHER,
        reply_markup=ReplyKeyboardMarkup(
            reply_keyboard,
            one_time_keyboard=True,
            input_field_placeholder=f"Type {Options.YES} or {Options.NO}",
...
def send_message_to_all_users(update: Update, context: CallbackContext):
    ...
    for user in fire.get_all_users():
        context.bot.send_message(
            chat_id=user.id, text=f"Here is a message from Samad 👇🏾\n{msg}"
        )
...
def run_bot():
    ...
    setup_conversation_handler = ConversationHandler(
        ...
        fallbacks=[
            CommandHandler(Commands.CANCEL, cancel),
    ...
    edit_conversation_handler = ConversationHandler(
        ...
        states={
            ...
            FIRST_NAME_EDIT_PROMPT: [
                MessageHandler(Filters.regex("^(Yes|No)$"), edit_first_name_prompt)
            ],
            LAST_NAME_EDIT_PROMPT: [
                MessageHandler(Filters.regex("^(Yes|No)$"), edit_last_name_prompt)
            ],
            HOSPITAL_NAME_EDIT_PROMPT: [
                MessageHandler(Filters.regex("^(Yes|No)$"), edit_hospital_name_prompt)
            ],
            SIGNATURE_EDIT_PROMPT: [
                MessageHandler(Filters.regex("^(Yes|No)$"), edit_signature_prompt)
            ],
        },
        fallbacks=[
            CommandHandler(Commands.CANCEL, cancel),
    ...
    samad_conversation_handler = ConversationHandler(
        ...
        states={SAM_MESSAGE: [MessageHandler(Filters.text, sam_message)]},
