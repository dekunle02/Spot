...
FIRST_NAME_EDIT, FIRST_NAME_EDIT_PROMPT, LAST_NAME_EDIT, LAST_NAME_EDIT_PROMPT, HOSPITAL_NAME_EDIT, HOSPITAL_NAME_EDIT_PROMPT, SIGNATURE_EDIT, SIGNATURE_EDIT_PROMPT = range(
    4, 12)
...
DELETE_USER = 21
TIMESHEET_START_DATE, TIMESHEET_START_TIME, TIMESHEET_DAYS, TIMESHEET_END_TIME = range(
    22, 26)
NIGHT_PROMPT, NIGHT_DAY, NIGHT_TIME, NIGHT_DURATION, NIGHT_REASON = range(
    26, 31)
...
def help(update: Update, context: CallbackContext):
    update.message.reply_text(
        text=Replies.HELP_MESSAGE, parse_mode=ParseMode.HTML)
...
def error(update: Update, context: CallbackContext) -> int:
    ...
    context.bot.send_message(
        chat_id=MY_TELEGRAM_ID, text=f"@{sender.first_name}-{sender.id}\n"+message, parse_mode=ParseMode.HTML)
    update.message.reply_text(
        text=Replies.ERROR_MESSAGE, parse_mode=ParseMode.HTML)
...
def last_name(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        text=Replies.HOSPITAL_NAME_ASK, parse_mode=ParseMode.HTML)
...
def edit(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            reply_keyboard, one_time_keyboard=True, input_field_placeholder=f"Type {Options.YES} or {Options.NO}")
    )
...
def edit_first_name_prompt(update: Update, context: CallbackContext) -> int:
    ...
    if prompt_answer == Options.YES:
        update.message.reply_text(
            Replies.EDIT_FIRST_NAME, reply_markup=ReplyKeyboardRemove())
...
def edit_last_name_prompt(update: Update, context: CallbackContext) -> int:
    ...
    if prompt_answer == Options.YES:
        update.message.reply_text(
            Replies.LAST_NAME_ASK, reply_markup=ReplyKeyboardRemove())
...
def edit_hospital_name_prompt(update: Update, context: CallbackContext) -> int:
    ...
    if prompt_answer == Options.YES:
        update.message.reply_text(
            Replies.EDIT_HOSPITAL_NAME, reply_markup=ReplyKeyboardRemove())
...
def edit_signature_prompt(update: Update, context: CallbackContext) -> int:
    ...
    if prompt_answer == Options.YES:
        update.message.reply_text(
            Replies.EDIT_SIGNATURE, reply_markup=ReplyKeyboardRemove())
        return SIGNATURE_EDIT
    else:
        user = fire.get_user_with_id(update.message.from_user.id)
        cd = context.user_data
        first_name = cd.get('first_name') if cd.get(
            'first_name') else user.first_name
        last_name = cd.get('last_name') if cd.get(
            'last_name') else user.last_name
        hospital_name = cd.get('hospital_name') if cd.get(
            'hospital_name') else user.hospital_name
        fire.update_user(
...
def delete_user(update: Update, context: CallbackContext) -> int:
    ...
    if prompt_answer == Options.YES:
        ...
        update.message.reply_text(
            Replies.DELETE_USER_FINAL, reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
    else:
        update.message.reply_text(
            text=Replies.DELETE_USER_SKIP, reply_markup=ReplyKeyboardRemove())
        return ConversationHandler.END
...
def timesheet_start_date(update: Update, context: CallbackContext) -> int:
    ...
    if not parser.string_is_valid_date(entered_date):
        update.message.reply_text(
            text=Replies.DATE_ERROR, parse_mode=ParseMode.HTML)
        return
    context.user_data['start_date'] = entered_date
    update.message.reply_text(
        Replies.TIME_ASK, quote=False, parse_mode=ParseMode.HTML)
...
def timesheet_start_time(update: Update, context: CallbackContext) -> int:
    ...
    if not parser.string_is_valid_time(entered_time):
        update.message.reply_text(
            text=Replies.TIME_ERROR, parse_mode=ParseMode.HTML)
        return
...
def timesheet_days(update: Update, context: CallbackContext) -> int:
    try:
        ...
        update.message.reply_text(
            Replies.NUMBER_INPUT_ERROR, parse_mode=ParseMode.HTML)
        return
...
def timesheet_end_time(update: Update, context: CallbackContext) -> int:
    ...
    start_time: datetime = parser.convert_string_into_date(
        date_string=entered_start_date, time_string=entered_start_time)
    end_time: datetime = start_time.replace(
        hour=int(end_hour), minute=int(end_minute)) + timedelta(days=int(day_count))
    timesheet = models.TimeSheet(
        user=user, shift_start=start_time, shift_end=end_time)
...
def night_prompt(update: Update, context: CallbackContext) -> int:
    ...
    if prompt_answer == Options.NO:
        ...
        context.bot.send_photo(
            chat_id=update.message.from_user.id, photo=photo_string)
        update.message.reply_text(
            text=Replies.TIMESHEET_MESSAGE, reply_markup=ReplyKeyboardRemove())
        context.bot.send_message(chat_id=MY_TELEGRAM_ID, text=f"{sender.id} {sender.first_name} made a timesheet!")    
        return ConversationHandler.END

    update.message.reply_text(Replies.DISTURBANCE_DAY_ASK,
...
def night_day(update: Update, context: CallbackContext) -> int:
    try:
        ...
        return
    update.message.reply_text(Replies.DISTURBANCE_TIME_ASK,
                              reply_markup=ReplyKeyboardRemove(), parse_mode=ParseMode.HTML)
    return NIGHT_TIME
...
def night_time(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        Replies.DISTURBANCE_DURATION_ASK, parse_mode=ParseMode.HTML)
...
def night_duration(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        Replies.DISTURBANCE_REASON_ASK, parse_mode=ParseMode.HTML)
...
def night_reason(update: Update, context: CallbackContext) -> int:
    ...
    time = timesheet.shift_start.replace(hour=int(hour), minute=int(
        min)) + timedelta(days=int(context.user_data['disturbance_day']))
...
def send_message_to_user(update: Update, context: CallbackContext):
    ...
    context.bot.send_message(chat_id=(user_id.strip()), text=f"Here is a message from Samad 👇🏾\n{msg}")
...
def send_message_to_all_users(update: Update, context: CallbackContext):
    ...
    for user in fire.get_all_users():
        context.bot.send_message(chat_id=user.id, text=f"Here is a message from Samad 👇🏾\n{msg}")
...
def run_bot():
    ...
    dispatcher.add_handler(CommandHandler (Commands.DIRECT, send_message_to_user))
    dispatcher.add_handler(CommandHandler (Commands.ALL, send_message_to_all_users))
